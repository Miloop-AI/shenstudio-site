...
- The header uses the founder's official logo (red S mark + black 「大道传媒 Shen Studio」, delivered 2026-09-23, source files in `assets/brand/`), embedded in the HTML as base64 by the build script. An earlier version referenced the logo by relative file path, which failed to render whenever the page was opened outside its own folder — hence the base64 rule in `CLAUDE.md`. The old black placeholder logo is in `archive/old-logo/`.
...
- Hero image is the founder's own banner from her YouTube channel, used untouched: the centre band YouTube itself shows on desktop (2048×338, `assets/banners/hero_strip.jpg`, embedded as base64), full width under the nav, with a 1546:423 crop on phones. It contains 「硅谷C位」 as well as 「沈琛琛 Cici Shen | 大道传媒」; the user chose to show it as it is even though the page's own brand is 大道传媒 (an earlier version that removed the 「硅谷C位」 line was reverted). Brand-level, so no per-post maintenance.
